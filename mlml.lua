...
local function setSpeed(enable)
    ...
    if speedEnabled then
        ...
        showNotification("✓ Speed ON", COLORS.SUCCESS)
    else
        showNotification("✗ Speed OFF", COLORS.ERROR)
    end
...
local function toggleLag()
    ...
    if isLagging then
        ...
        if not target then
            isLagging = false
            showNotification("✗ NO TARGET", COLORS.ERROR)
            return
        end
        
        showNotification("▶ LAGGER STARTED", COLORS.ACCENT)
...
subtitle.Text = "By rxme | 40% = flash tp/50 speed"
